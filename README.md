...
The agent modules under `backend/app/agents/` can be called directly from
Python or the command line.

Examples:

```bash
# General CSV profiling (CLI)
python -m backend.app.agents.general_agent path/to/your.csv --datetime-col date_time

# Traffic-specific profiling
python -m backend.app.agents.traffic_agent path/to/your.csv --datetime-col date_time
```

Both emit JSON files describing their respective profiles.

---

## 6. Notes & limitations

- The backend keeps an in-memory `DATASETS` store keyed by `dataset_id`.
  This is fine for a demo or paper prototype but **not** for production.
- Error handling is intentionally lightweight; malformed CSVs may raise
  exceptions in the ingestion step.
- The RAG configuration file (`config/rag_demo_config.json`) is for
  documentation only; it is not used in the running application.

For the EDBT paper, this structure should make it clear how the different
pieces (ingestion, profiling, insight generation, and Q/A) fit together
without being tied to a single hard-coded "demo" layout.
